...
export class MainService {
  ...
  private handleCreationBucket(bucketSize: number): void {
    const bucketArray: Bucket[] = Array.from(
    ...
    this.pages.forEach((page: Page) => {
      page.getTuples().forEach((tuple: Tuple) => {
        const hashKeyValue = this.hashFunction.hashT(
          tuple.getValueOfData(),
          this.numberMaxOfBuckets
        );
        bucketArray[hashKeyValue].addMapping(
          tuple.getValueOfData(),
          page.getPageNumber()
        );
      });
  ...
  handleCreationPagesWithBuckets(
    bucketSize: number,
    pageSize: number,
    table: Table
  ): void {
    ...
    this.numberMaxOfBuckets = Math.ceil(
      table.getListOfTuples().length / bucketSize
    );
  ...
  getPagesVisitedByTableScan(value: string): number {
    ...
    for (const page of this.pages) {
      ...
      const indexFound: Tuple = page
        .getTuples()
        .find((t) => t.getValueOfData() === value);
